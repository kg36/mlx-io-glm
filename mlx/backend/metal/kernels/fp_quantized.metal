...
// MXFP8 counterpart of the exact dual-row QMV. The loop and accumulation for
// each row match fp_qmv_fast_impl<bfloat16_t, 32, 8>; only the packed weight
// decode is shared between the two independent accumulators. Grid z permits
// grouped attention projections without a second kernel or policy path.
[[kernel]] void dsv4_mxfp8_two_row_dual_bf16(
    const device uint32_t* weight [[buffer(0)]],
    const device uint8_t* scales [[buffer(1)]],
    const device bfloat16_t* x [[buffer(2)]],
    device bfloat16_t* output [[buffer(3)]],
  ...
  const ulong input_group_stride = 2ul * ulong(in_vec_size);
  const ulong output_group_stride = 2ul * ulong(out_vec_size);
