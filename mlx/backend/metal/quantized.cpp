...
inline int get_qmv_batch_limit(int D, int O, metal::Device& d) {
  ...
  // Retain the pre-v0.32.1 M5 dispatch used by LivSeek's qualified outputs.
...
void qmm_nax(
  ...
  int wm = 2;
  int wn = 2;
  // Preserve LivSeek's qualified small-M accumulation geometry. The bm32
  // specialization can change BF16 rounding and greedy token selection.
  int bm = 64;
  int bn = 64;
  int bk = 64;
